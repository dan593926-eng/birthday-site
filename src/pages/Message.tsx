  ...
  return (
    <div className="relative min-h-[100dvh] w-full flex flex-col items-center justify-center px-6 py-24">
      ...
      >
        <motion.span
          variants={item}
          className="text-xs tracking-[0.3em] text-[var(--primary)] uppercase"
        >
          {siteData.personName}
        </motion.span>
        ...
        <motion.h2
          variants={item}
          className="font-serif-display italic text-2xl sm:text-3xl md:text-5xl text-white leading-snug"
        >
        ...
        <div className="mt-2 flex flex-col gap-4">
          {siteData.message.paragraphs.map((p, i) => (
            <motion.p
              key={i}
              variants={item}
              className="text-white/60 text-sm md:text-lg font-light leading-relaxed"
            >
              {p}
            </motion.p>
          ))}
